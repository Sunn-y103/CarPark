...
const TABS: Tab[] = [
  { id: 'maps', label: 'Maps', icon: require('../assets/map.png') },
  { id: 'history', label: 'History', icon: require('../assets/history.png') },
  { id: 'home', label: 'Home', icon: require('../assets/home.png') },
  { id: 'wallet', label: 'Wallet', icon: require('../assets/wallet.png') },
  { id: 'profile', label: 'Profile', icon: require('../assets/userprofile.png') },
];
...
const CustomBottomNavigation: React.FC<CustomBottomNavigationProps> = ({
  ...
  function getContentWidth(tabId: string) {
    ...
    switch (tabId) {
      case 'home': return baseIconWidth + textPadding + 38; // "Home" width
      case 'maps': return baseIconWidth + textPadding + 36; // "Maps" width  
      case 'history': return baseIconWidth + textPadding + 48; // "History" width
      case 'wallet': return baseIconWidth + textPadding + 40; // "Wallet" width
  ...
  React.useEffect(() => {
    if (parentActiveTab && parentActiveTab !== activeTab) {
      ...
      const tabIndex = TABS.findIndex(tab => tab.id === parentActiveTab);
      if (tabIndex >= 0) {
